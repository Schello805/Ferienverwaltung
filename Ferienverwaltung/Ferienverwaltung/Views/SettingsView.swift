...
extension SettingsView {
    private func exportData() {
        ...
        let fileURL = docsDir.appendingPathComponent("ferienverwaltung_export.json")
        do {
            try jsonData.write(to: fileURL, options: .atomic)
            // Prüfen, ob Datei existiert und lesbar ist
            guard FileManager.default.fileExists(atPath: fileURL.path),
                  FileManager.default.isReadableFile(atPath: fileURL.path) else {
                print("[Export] Datei existiert nicht oder ist nicht lesbar. Fallback auf Data-Export.")
                let av = UIActivityViewController(activityItems: [jsonData], applicationActivities: nil)
            ...
            // Datei existiert und ist lesbar: normal teilen
            ...
            print("[Export] Export fehlgeschlagen: \(error). Fallback auf Data-Export.")
            let av = UIActivityViewController(activityItems: [jsonData], applicationActivities: nil)
            if let scene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
               let root = scene.windows.first?.rootViewController {
                root.present(av, animated: true)
...
// MARK: - SwiftUI Import-Button (modern, robust)
struct ImportExportButton: View {
    ...
    var body: some View {
        ...
        .alert("Import fehlgeschlagen", isPresented: Binding<Bool>(
            get: { importError != nil },
        ...
        .alert("Import erfolgreich", isPresented: $showImportSuccess, actions: {
            Button("OK", role: .cancel) {}
        }, message: {
